...
first_beam_angle = 0.25*np.pi
first_center_angle = -0.5*np.pi #horizontal right out of trap = 0 rad --> to turn right, use negative values
second_center_angle = 1.25*np.pi
second_beam_angle = 1.73*np.pi
...
#Debye-Waller Factor
debye_waller = dw.DW_two_complete(first_center_angle, first_beam_angle, omega_z, omega_x, omega_y, Phonons_rocking_x, Phonons_rocking_y, Phonons_breath)
...
exp_gtwo = cont.total_contrast(bin_number,saturation_param,first_center_angle, first_beam_angle,slit_width,ion_distance,debye_waller)
print(exp_gtwo)
...
x = np.cos(first_center_angle)*512.+512
y = -np.sin(first_center_angle)*512.+512
x2 = np.cos(first_beam_angle)*512.+512
y2 = -np.sin(first_beam_angle)*512.+512
x3 = np.cos(second_center_angle)*512.+512
y3 = -np.sin(second_beam_angle)*512.+512
x4 = np.cos(second_beam_angle)*512.+512
y4 = -np.sin(second_center_angle)*512.+512
plt.plot([512,x],[512,y], color="blue", label="outgoing beam")
plt.plot([512,x2],[512,y2], color="red", label="incoming beam")
plt.plot([512,x3],[512,y3], color="green", label="incoming beam")
plt.plot([512,x4],[512,y4], color="black", label="outgoing beam")
...
plt.show()
plt.clf()
cont.visualize_gtwo(first_center_angle, first_beam_angle,saturation_param,ion_distance,debye_waller)